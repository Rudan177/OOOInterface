...
this.themeOverrides 对象用于存储当前主题对 logo / font / wallpaper 的专用配置，与用户普通设置区分开，确保主题切换时能正确应用和清理。
...
handleThemeUpload() 支持用户导入自定义主题文件，兼容 JSON 和 JS 两种格式，导入后存入 settings.customThemes 持久化保存。
...
LOGO 会先加载本地 LOGO，随后检测云端连通性，并在云端 LOGO 加载完毕后显示云端 LOGO。

location 允许直接填写云端路径，无需本地相对路径。但如果同时存在本地和云端 LOGO，则location必须填写本地相对路径，云端链接填写在specialStyle中online。

若存在dark，但不存在onlineDark，则暗色模式使用dark。
...
wallpaperFill 对应设置页壁纸面板"填满"开关，请填写true（开）false（关）。留空则遵循用户设定。建议打开（不想被用户骂的话）。
...
1. 引用内置配色colorGroup: "cjs" ：直接引用系统内置的配色方案：
...
| custom | 自定义 | — |
2. 主题自带完整配色colorGroup: "add" ：自带一套完整的配色配置，不依赖内置方案：
...
在使用moreStyle自定义 CSS 时，请确保您有必要的 CSS 知识，且已熟悉 OOOInterface CSS内容和规范。

moreStyle 中的 CSS 代码关联 style.css ，因此无法影响高级视觉效果。
...
那么我们便可 moreStyle 中添加自定义 CSS 代码将圆角更改为定值 50px 。
...
## 测试方式
 ...
 OOOInterface 29 测试版现已发布，您可以在测试版中测试您的主题。

为确保稳定，建议：


测试时使用"开发者方案"导入，随后通过多次重置验证效果。

若无法确保自己精通CSS，请不要乱改 more 在 OOOInterface 的 CSS 上拉屎，这会影响您自身使用体验。

自己写配色，请一定要检查深浅色、高级视觉效果下的显示效果。若发现有逻辑错误，请第一时间联系 wyjcrtu@proton.me。
