(function () {
    ...
    (function () {
        ...
        var rafId = null;
        var touchStartY = 0;
        var scrollToTopGen = 0; // 记录回顶动画的代次，用于结束时保留正确高度
        ...
        function onTouchStart(e) {
            ...
            // 触摸在侧边栏按钮上时，不拦截（让点击正常工作）
            if (target.closest('.tab-sidebar-btn')) {
                touchTracking = false;
                return;
            }
            e.preventDefault();
        ...
        function onTouchMove(e) {
            ...
            // 速度计算：归一化到 px/frame（≈16ms），放大系数改为 0.5 避免惯性过大
        ...
        window.scrollToTop = function() {
            ...
            isAnimating = false;
            animGen++;
            var storedVScroll = vScroll;
            // 保存当前 bodyFixed 状态
            var wasBodyFixed = bodyFixed;
            vScroll = 0;
            targetVScroll = 0;
            contentScroll = 0;
            inputVelocity = 0;
            ...
            (function() {
                ...
                render();
                initCodeBlocks();
                // 开始回顶动画：保持 bodyFixed 在固定状态，内容在框内平滑滚回顶部
                vScroll = storedVScroll;
                targetVScroll = 0;
                bodyFixed = wasBodyFixed;  // 与切换前保持一致
                bodyFixedStartScroll = SCROLL_DISTANCE;
                bodyFixedStartTop = bodyEl.getBoundingClientRect().top;
                scrollToTopGen = animGen; // 标记本轮动画归属
                isAnimating = true;
        ...
        function tick() {
            ...
            if (touchTracking) {
                ...
                if (Math.abs(diff) < 0.5 && Math.abs(inputVelocity) < 0.5) {
                    vScroll = targetVScroll;
                    // 动画结束时，若回到顶部则彻底清理 bodyFixed 状态
                    if (vScroll < 1) {
                        // 先保留当前非固定高度（避免渲染时 body height 跳变）
                        var cs = getComputedStyle(document.documentElement);
                        var ftGap = parseFloat(cs.getPropertyValue('--r-body-ft-gap')) || 20;
                        var initFooterH = footerEl.offsetHeight || 24;
                        var bodyRectNow = bodyEl.getBoundingClientRect();
                        var initMaxH = window.innerHeight - bodyRectNow.top - initFooterH - ftGap;
                        var topHeight = Math.max(initMaxH, 200);
                        // 清除所有 bodyFixed 状态和样式
                        bodyFixed = false;
                        bodyFixedStartTop = 0;
                        bodyFixedStartScroll = 0;
                        bodyEl.style.position = '';
                        ...
                        contentEl.scrollTop = 0;
                        contentScroll = 0;
                        // 立即设置正确的非固定高度，避免渲染时出现高度跳变
                        bodyEl.style.height = topHeight + 'px';
                    }
                    render();
                    isAnimating = false;
                    return;
